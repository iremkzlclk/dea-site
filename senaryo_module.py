...
def optimum_senaryo_bul(sonuc: dict, sinir_araligi: tuple = (-0.30, 0.30),
                         sadece_anlamli: bool = True, maks_deneme: int = 60) -> dict:
    """
    SciPy (Nelder-Mead, sinirli -- YEREL ama turevsiz arama) ile, TUM
    anlamli girdileri AYNI ANDA, YONU ONCEDEN VARSAYMADAN (hem artis hem
    azalis serbestce denenir), TUM DMU'lara AYNI (uniform) yuzdeyle
    uygulayarak, ORTALAMA M'yi MAKSIMIZE eden kombinasyonu arar.

    NEDEN YEREL ARAMA (Nelder-Mead) -- KURESEL DEGIL: her deneme TAM BIR
    DEA+Malmquist cozumu gerektirir (~0.5 saniye/deneme) -- kuresel arama
    (orn. differential_evolution) yuzlerce-binlerce deneme gerektirdigi
    icin dakikalarca surer. Nelder-Mead, panelin onerdigi yonden BASLAYIP
    (sicak baslangic) 60 civari denemeyle YAKINSAR -- kucuk boyutlu (2-4
    degisken), nispeten pürüzsüz bu problem icin pratikte yeterlidir.
    ONEMLI SINIRLAMA: yerel arama oldugu icin, KURESEL en iyiyi bulmayi
    GARANTI ETMEZ (baslangic noktasina bagli bir yerel optimuma takilabilir)
    -- bu yuzden "tum_denemeler" tablosu da donduruluyor, boylece aramanin
    gercekten yakinsadigi (M'nin duzlestigi) gozle de dogrulanabilir.

    Panel katsayisinin isareti burada bir ON-KOSUL DEGIL, sadece HANGI
    girdilerin karar degiskeni olacagini (anlamlilik filtresi) belirler --
    YON, dogrudan GERCEK DEA/Malmquist sonucuna gore optimizor tarafindan
    bulunur. Bu, panelin "artir" dedigi ama DEA'nin yapisal olarak
    cezalandirdigi durumlarda, optimizorun DEA'yi dinlemesini saglar.

    sinir_araligi: her girdi icin izin verilen yuzde degisim araligi
                   (varsayilan -%30 ile +%30 arasi).
    sadece_anlamli: True ise sadece p<alpha olan girdiler karar degiskeni
                    olur (anlamsiz girdiler %0'da sabit kalir).

    Returns: dict -- karar_degiskenleri (liste), en_iyi_yuzdeler (dict),
             en_iyi_M, tum_denemeler (DataFrame, her denemenin kaydi --
             yakinsama grafigi icin), gelecek (en iyi kombinasyonun tam
             gelecek_donem_analizi_manuel ciktisi)
    """
    from scipy.optimize import minimize

    veri = sonuc["veri"]
    girdi_cols = veri["girdi_cols"]
    ...
    if sadece_anlamli:
        karar_degiskenleri = [g for g in girdi_cols if g in girdi_bilgisi.index and girdi_bilgisi.loc[g, "anlamli_mi"]]
    else:
        karar_degiskenleri = [g for g in girdi_cols if g in girdi_bilgisi.index]

    if not karar_degiskenleri:
        return {"basarili": False, "mesaj": "Optimize edilecek (anlamli) girdi bulunamadi."}

    denemeler = []
    onbellek = {}

    def _objektif(x):
        """x: her karar degiskeni icin yuzde degisim (orn. -0.10 = %10 azalt).
        Negatif ortalama M dondurur (minimize eder). Ayni nokta tekrar
        denenirse (Nelder-Mead sik sik yapar) onbellekten okunur."""
        anahtar = tuple(round(float(v), 4) for v in x)
        if anahtar in onbellek:
            return onbellek[anahtar]

        girdi_secimleri = {}
        for deger, g in zip(x, karar_degiskenleri):
            girdi_secimleri[g] = {"yon": 1 if deger >= 0 else -1, "yuzde": abs(float(deger))}
        try:
            gelecek = gelecek_donem_analizi_manuel(sonuc, girdi_secimleri)
            m_ort = float(gelecek["malmquist"]["M"].mean())
        except Exception:
            m_ort = 1.0

        denemeler.append({**{g: round(v, 4) for g, v in zip(karar_degiskenleri, anahtar)}, "Ortalama_M": round(m_ort, 4)})
        sonuc_deger = -m_ort
        onbellek[anahtar] = sonuc_deger
        return sonuc_deger

    # Sicak baslangic: panelin onerdigi yonde, %5 ile basla
    x0 = np.array([
        (1 if girdi_bilgisi.loc[g, "onerilen_yon"] > 0 else -1) * 0.05
        for g in karar_degiskenleri
    ])
    sinirlar = [sinir_araligi for _ in karar_degiskenleri]

    sonuc_opt = minimize(
        _objektif, x0, method="Nelder-Mead", bounds=sinirlar,
        options={"maxfev": maks_deneme, "xatol": 0.01, "fatol": 0.0005, "adaptive": True},
    )

    en_iyi_yuzdeler = {g: round(float(v), 4) for g, v in zip(karar_degiskenleri, sonuc_opt.x)}
    en_iyi_M = -float(sonuc_opt.fun)

    en_iyi_secim = {g: {"yon": 1 if v >= 0 else -1, "yuzde": abs(v)} for g, v in en_iyi_yuzdeler.items()}
    gelecek_en_iyi = gelecek_donem_analizi_manuel(sonuc, en_iyi_secim)

    return {
        "basarili": True,
        "karar_degiskenleri": karar_degiskenleri,
        "en_iyi_yuzdeler": en_iyi_yuzdeler,
        "en_iyi_M": round(en_iyi_M, 4),
        "tum_denemeler": pd.DataFrame(denemeler),
        "gelecek": gelecek_en_iyi,
        "sinir_araligi": sinir_araligi,
        "yakinsadi_mi": bool(sonuc_opt.success),
